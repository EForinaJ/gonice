...
// arModel is a active record design model for table sys_tag operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of sys_tag.
	Table = "sys_tag"
	// Model is the model object of sys_tag.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table sys_tag.
	Columns = struct {
		TagId       string //                                  
        TagName     string // 标签名称                         
        TagModule   string // 所属模块                         
        IsHot       string // 是否为推荐标签(0 为 否，1为 是)  
        CreateBy    string // 创建者                           
